...
SCHEDULER.every '60m', :first_in => 0 do |job|
  ...
  project_list_table_data = capture_html.xpath("//table/tr")

  project_details = project_list_table_data.collect do |row|
    detail = {}
    [
      [:project, 'td[1]/text()'],
      [:primary, 'td[2]/text()'],
  ...
  total_projects_count = project_details.count
  ...
  grouped_by_test_suite = project_details.group_by {|item| item[:test_suite]}
  if grouped_by_test_suite.any? {|k,v| k.include? "lightgreen"} then
    ...
    test_suites_complete_pct = ((test_suites_complete.to_f / project_details.length.to_f) * 100).round(2)
  end

  grouped_by_unit_tests = project_details.group_by {|item| item[:unit_tests]}
  if grouped_by_unit_tests.any? {|k,v| k.include? "lightgreen"} then
    ...
    unit_tests_complete_pct = (unit_tests_complete.to_f / project_details.length.to_f).round(2)
  end

  grouped_by_functional_tests = project_details.group_by {|item| item[:functional_tests]}
  if grouped_by_functional_tests.any? {|k,v| k.include? "lightgreen"} then
    ...
    functional_tests_complete_pct = (functional_tests_complete.to_f / project_details.length.to_f).round(2)
  end

  grouped_by_load_tests = project_details.group_by {|item| item[:load_tests]}
  if grouped_by_load_tests.any? {|k,v| k.include? "lightgreen"} then
    ...
    load_tests_complete_pct = (load_tests_complete.to_f / project_details.length.to_f).round(2)
  end

  grouped_by_performance_tests = project_details.group_by {|item| item[:performance_tests]}
  if grouped_by_performance_tests.any? {|k,v| k.include? "lightgreen"} then
    ...
    performance_tests_complete_pct = (performance_tests_complete.to_f / project_details.length.to_f).round(2)
  end

  grouped_by_accessibility_tests = project_details.group_by {|item| item[:accessibility_tests]}
  if grouped_by_accessibility_tests.any? {|k,v| k.include? "lightgreen"} then
    ...
    accessibility_tests_complete_pct = (accessibility_tests_complete.to_f / project_details.length.to_f).round(2)
  end

  grouped_by_security_tests = project_details.group_by {|item| item[:security_tests]}
  if grouped_by_security_tests.any? {|k,v| k.include? "lightgreen"} then
    ...
    security_tests_complete_pct = (security_tests_complete.to_f / project_details.length.to_f).round(2)
  end

  grouped_by_localization_tests = project_details.group_by {|item| item[:localization_tests]}
  if grouped_by_localization_tests.any? {|k,v| k.include? "lightgreen"} then
    ...
    localization_tests_complete_pct = (localization_tests_complete.to_f / project_details.length.to_f).round(2)
  end

  total_automation_possibilities = project_details.length * 7 #7 being the number of test automation types
